...
export class RunListenerCycle {
  ...
  /** Manda el brief solo (sin que se pida) una vez al día, pasada la hora configurada. */
  async maybeSendDailyBrief(state) {
    ...
    const text = await this.dailyBrief.generate().catch((e) => {
      this.logger.log(`brief diario falló: ${e.message}`);
      return null;
    });
    if (!text) return false;

    await this.notifier.notify(`☀️ *Brief del día*\n\n${text}`).catch((e) => this.logger.log(`notify (brief diario) falló: ${e.message}`));
